...
def create_files(
    ...
    try:
        ...
        response = FileResponse(
            path=path, filename=fn, media_type=media_type, headers=headers
        )
        return response
    finally:
        aos.remove(path)
...
def filter_conjugations(
    root: List[str] = Query(None),
    option: List[str] = Query(None),
...
def read_conjugations_from_file(
    root: List[str] = Query(None),
    option: List[str] = Query(None),
    agent: List[str] = Query(None),
    patient: List[str] = Query(None),
    ...
    if root or agent or patient or option:
        ...
        return FileResponse(
            path=os.path.join(DATA_PATH, WWLANG, "conjugations.json.gz"),
            headers=headers,
        )


@router.post("/files", tags=["conjugations"])
def create_files_from_file_request(
    root: List[str] = Query(None),
    option: List[str] = Query(None),
    agent: List[str] = Query(None),
    patient: List[str] = Query(None),
    ...
    return create_files(
        conjugations=filtered, file_type=file_type, tiers=tiers, settings=settings
    )
